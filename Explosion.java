import greenfoot.*;  // (World, Actor, GreenfootImage, Greenfoot and MouseInfo)
/**
 * @author (Joseph Pepe) 
 * @version (2.0)
 */
public class Explosion extends Actor
{
    /** How many images should be used in the animation of the explosion */
    protected final static int IMAGE_COUNT = 10;
    
    /** 
     * The images in the explosion. This is static so the images are not
     * recreated for every object (improves performance significantly).
     */
    protected static GreenfootImage[] images;
    
    /** Current size of the explosion */
    protected int currentImageSize = 0;
    
    /** How much do we increment the index in the explosion animation. */
    protected int increment = 1;
    
    public Explosion() {
        initialiseImages();
        setImage(images[0]);  
    }    
    /** 
     * Create the images for explosion.
     */
    public synchronized static void initialiseImages() 
    {
        if(images == null) 
        {
            GreenfootImage baseImage = new GreenfootImage("explosion-big.png");
            int maxSize = baseImage.getWidth();
            int delta = maxSize / IMAGE_COUNT;
            int size = 0;
            images = new GreenfootImage[IMAGE_COUNT];
            for(int i=0; i < IMAGE_COUNT; i++) {
                size = size + delta;
                images[i] = new GreenfootImage(baseImage);
                images[i].scale(size, size);
            }
        }
    }
    /**
     * EXPLODE!
     */
    public void act()
    { 
        changeImage();
        remove();
    }
    private void changeImage()
    {
        setImage(images[currentImageSize]);
        currentImageSize += increment;
        if(currentImageSize >= IMAGE_COUNT) 
        {
            increment = -increment;
            currentImageSize += increment;
        }
    }
    private void remove()
    {
        if(currentImageSize < 0) 
        {
            getWorld().removeObject(this);
            Greenfoot.playSound("Explosion.wav");
        }
    }
}
